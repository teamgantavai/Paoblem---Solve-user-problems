...
export async function generateMetadata({ params }: CategoryPageProps): Promise<Metadata> {
  ...
  if (!type) {
    return {
      title: 'Category Not Found | Paoblem',
    };
  }
...
export default async function CategoryPage({ params, searchParams }: CategoryPageProps) {
  ...
  const { posts, count } = await getCategoryPosts(type, page);
        ...
        <main className="center-feed">
          ...
          {totalPages > 1 && (
            <nav style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '1.5rem' }} aria-label="Pagination Navigation">
              {hasPrev ? (
                <Link href={`/category/${category}?page=${page - 1}`} rel="prev" style={{ fontSize: '0.82rem', padding: '6px 12px', background: 'var(--bg-card)', border: '1px solid var(--border-color)', borderRadius: '8px', color: 'var(--text-main)', textDecoration: 'none' }}>
                  ← Previous
                </Link>
              ) : (
              ...
              {hasNext ? (
                <Link href={`/category/${category}?page=${page + 1}`} rel="next" style={{ fontSize: '0.82rem', padding: '6px 12px', background: 'var(--bg-card)', border: '1px solid var(--border-color)', borderRadius: '8px', color: 'var(--text-main)', textDecoration: 'none' }}>
                  Next →
                </Link>
              ) : (
